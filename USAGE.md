...
> Review（顏色、間距、元件）都還在 roadmap 上，做好的只有字級與字重。另外它**不支援 iOS
> Dynamic Type / Android 字級可調**——使用者把系統字級調大之後，整套字級表會失效。給年齡
> 分布偏高的產品用之前，這件事要先想清楚。
...
- **不支援系統字級調整。** iOS Dynamic Type 與 Android 字級可調都沒有納入，使用者調大字級
  後整套會失效。
